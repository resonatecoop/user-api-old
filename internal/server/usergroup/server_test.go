	...
	Describe("GetUserGroup", func() {
		Context("with valid uuid", func() {
			It("should respond with user_group if it exists", func() {
				...
				Expect(len(resp.TrackGroups[0].Tracks)).To(Equal(1))
				Expect(resp.TrackGroups[0].Tracks[0].Id).To(Equal(newTrack.Id.String()))
				...
				Expect(len(resp.FeaturedTrackGroup.Tracks)).To(Equal(1))
				Expect(resp.FeaturedTrackGroup.Tracks[0].Id).To(Equal(newTrack.Id.String()))
